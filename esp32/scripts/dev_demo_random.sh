...
# Build and upload the random demo firmware, then monitor serial output.

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
FQBN="esp32:esp32:esp32s3:CDCOnBoot=cdc"
BUILD_DIR="$ROOT/build_demo_random"
DEMO_DIR="$ROOT/firmware_demo_random"
...
echo "==> Compiling random demo firmware..."
rm -rf "$BUILD_DIR"
arduino-cli compile \
  --fqbn "$FQBN" \
...
echo "==> Uploading random demo firmware..."
arduino-cli upload \
  --fqbn "$FQBN" \
  -p "$SERIAL_PORT" \
  --input-dir "$BUILD_DIR"
